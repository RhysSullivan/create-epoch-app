...
const createQueryAtom = (
	...
	return runtime.atom(
		Effect.gen(function* () {
			...
			const encodedExit = yield* client.query(convexFn, payload as Record<string, unknown>);
...
const createSubscriptionAtom = (
	...
	return runtime.atom(
		Stream.unwrap(
			Effect.gen(function* () {
				const client = yield* ConvexClient;
				return client.subscribe(convexFn, payload as Record<string, unknown>).pipe(
...
const createMutationFn = (
	...
	return runtime.fn<unknown>()(
		Effect.fnUntraced(function* (payload) {
			...
			const encodedExit = yield* client.mutation(convexFn, fullPayload as Record<string, unknown>);
			const exit = decodeExit(encodedExit as ExitEncoded);
			if (Exit.isSuccess(exit)) {
				return exit.value;
			}
...
const createActionFn = (
	...
	return runtime.fn<unknown>()(
		Effect.fnUntraced(function* (payload) {
			const client = yield* ConvexClient;
			const fullPayload = { ...getShared(), ...(payload as object) };
			const encodedExit = yield* client.action(convexFn, fullPayload as Record<string, unknown>);
			const exit = decodeExit(encodedExit as ExitEncoded);
			if (Exit.isSuccess(exit)) {
				return exit.value;
			}
	...
	return runtime.pull(
		Stream.paginateChunkEffect(null as string | null, (cursor) =>
			Effect.gen(function* () {
				...
				const encodedExit = yield* client.query(convexFn, fullPayload as Record<string, unknown>);
				const exit = decodeExit(encodedExit as ExitEncoded);
